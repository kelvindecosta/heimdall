...
def color(string, rgb):
    return f"{fg(*rgb)}{string}{fg.rs}"
...
for choice in [x.stem for x in directory.glob("*")]:
    choice_index = index.setdefault(choice, defaultdict())
    for scene_id in [
        x.stem[: -1 * len("-ortho")] for x in (directory / choice / "images").glob("*")
    ]:
        sample_set_index = None

        # Set the sample set
        for sample_set in dataset:
            if scene_id in dataset[sample_set]:
                sample_set_index = choice_index.setdefault(sample_set, [])
                break

        # Set whether the label exists
        sample_set_index.append(scene_id)

for choice in index:
    index_file = (directory / choice / "index.json").as_posix()
    with open(index_file, "w") as fd:
        json.dump(index[choice], fd, indent=2)
